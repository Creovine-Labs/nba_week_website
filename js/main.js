...
(function () {
    ...
    // Create flip-card HTML for a single digit
    ...
    // Update a single flip card with animation
    function updateFlipCard(card, newDigit) {
        ...
        // Set back to show OLD value (it will flip away)
        backSpan.textContent = current;
        // Set top to new value (revealed after flip)
        topSpan.textContent = newDigit;
        // Bottom shows new value
        ...
        // After animation, reset back to show new value
        setTimeout(function () {
            card.classList.remove('flipping');
    ...
    // Update all digits in a container
    ...
    const revealElements = document.querySelectorAll(
        ...
        '.schedule-item, .stat-card, .countdown-wrapper, ' +
        '.testimonial-card, .press-quote, .faq-item, ' +
        '.sponsor-tier, .hero-content > *, .hero-visual, ' +
    ...
    // Add stagger-children to grid containers
    document.querySelectorAll(
        '.speakers-grid, .events-grid, .pricing-grid, ' +
        '.testimonials-grid, .stats-grid, .press-quotes'
